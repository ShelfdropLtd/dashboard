...
export default async function OrderDetailPage({ params }: OrderDetailPageProps) {
  ...
  const { data: order } = await supabase
    ...
    .single() as { data: { id: string; po_number: string; order_date: string; status: string; warehouse: string; carrier: string | null; tracking_number: string | null; notes: string | null; order_lines: Array<{ id: string; sku: string; product_name: string; quantity_cases: number; case_price: number; line_total: number }> } | null }
                      ...
                      {index > 0 && (
                        <div
                          className={`flex-1 h-1 ${
                            index <= currentStatusIndex ? 'bg-brand-accent' : 'bg-gray-200'
                          }`}
                        />
                      )}
                      <div
                        className={`
                          ...
                          ${isCompleted ? 'bg-brand-accent text-white' : 'bg-gray-200 text-gray-500'}
